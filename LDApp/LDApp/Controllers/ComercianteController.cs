    ...
    public class ComercianteController : Controller
    {
        ...
        public String GetUtilidadePublica(String Tipo)
        {
            try
            {
                ...
                using (StreamReader CsvReader = new StreamReader(Server.MapPath("/docs/UtilidadePublica.csv"), Encoding.Default, true))
                {

                    while ((line = CsvReader.ReadLine()) != null)
                    {
                        string[] vars = line.Split(',');
                        if (Tipo.Equals(vars[2]))
                        {
                            UtilidadePublica utilidade = new UtilidadePublica();
                            utilidade.Descricao = vars[0];
                            utilidade.Telefone = vars[1];
                            utilidade.Tipo = vars[2];
        ...
        public String GetTelefonePrefeitura()
        {
            return GetUtilidadePublica("PREFEITURA");
        }

        public String GetTelefoneSantaCasa()
        {
            return GetUtilidadePublica("SANTACASA");
        }

        public String GetTelefoneCAMARA()
        {
            return GetUtilidadePublica("CAMARA");
        }

        public String GetTelefonePolicia()
        {
            return GetUtilidadePublica("POLICIA");
        }
        ...
        // GET: /Comerciante/Create
        public String GetHorarioOnibus(String Tipo)
        {
            try
            {
                String line = "";
                List<Transporte> utilidades = new List<Transporte>();
                using (StreamReader CsvReader = new StreamReader(Server.MapPath("/docs/HorariosOnibus.csv"), Encoding.Default, true))
                {
                    ...
                    while ((line = CsvReader.ReadLine()) != null)
                    {
                        ...
                        if (Tipo.Equals(vars[2]))
                        {
                            Transporte utilidade = new Transporte();
                            utilidade.OrigemDestino = vars[0];
                            utilidade.Dias = vars[1];
                            utilidade.Empresa = vars[2];
                            utilidades.Add(utilidade);
                        }
        ...
        public String GetBassamar()
        {
            return GetHorarioOnibus("BASSAMAR");
        }

        public String GetVimara()
        {
